...
def show_admin_controls():
    ...
    if st.session_state.role == "Admin":
        ...
        if st.sidebar.button("Clear User Data 🗑️", key="clear_user_data", help="Clear all data for selected user"):
            if selected_user:
                user_id = get_user_id(selected_user)
                clear_user_data(user_id)
                st.sidebar.success(f"Data cleared for {selected_user}!")
        if st.sidebar.button("Clear All Data 🧹", key="clear_all_data", help="Clear all user data"):
            clear_all_data()
